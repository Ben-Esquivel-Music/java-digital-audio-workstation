...
3. **Include the license file** (`LICENSE`, `COPYING`, or equivalent) in the library's source directory under `lib/`. The `generate-third-party-notices.sh` script auto-discovers these files to produce `THIRD_PARTY_NOTICES.md`. For richer metadata (display name, version, website), add corresponding entries to the `KNOWN_KEYS` and `KNOWN_VALS` arrays in that script.

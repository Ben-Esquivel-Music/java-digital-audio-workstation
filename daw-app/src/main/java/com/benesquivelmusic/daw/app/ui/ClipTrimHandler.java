...
final class ClipTrimHandler {
     ...
     * (CONTROL_SYNCHRONIZATION_DESIGN_BOOK §9). This is a plain data carrier of
     * functional dependencies the owning {@link ClipInteractionController} binds
     * straight to its own state and the {@link ArrangementCanvas}, not a
     * cross-surface cascade seam.
     *
    ...
    EdgeHit hitTestEdge(double x, double y) {
        ...
        for (AudioClip clip : track.getClips()) {
            double leftEdgeX = (clip.getStartBeat() - deps.scrollXBeats().getAsDouble()) * deps.pixelsPerBeat().getAsDouble();
            double rightEdgeX = (clip.getEndBeat() - deps.scrollXBeats().getAsDouble()) * deps.pixelsPerBeat().getAsDouble();
    ...
    void completeTrim(double x) {
        ...
        if (startChanged || durationChanged) {
            deps.undoManager().get().execute(new ClipEdgeTrimAction(
    ...
    private double beatThreshold() {
        return EDGE_THRESHOLD_PIXELS / deps.pixelsPerBeat().getAsDouble();
    }
}

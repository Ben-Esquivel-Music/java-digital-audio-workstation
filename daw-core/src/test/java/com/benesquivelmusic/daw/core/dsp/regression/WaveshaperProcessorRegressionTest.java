...
/**
 * Golden-file regression test for {@link WaveshaperProcessor}. Exercises the
 ...
 * goldens with {@code mvn -Pdsp-rebaseline test -Dtest=WaveshaperProcessorRegressionTest}.</p>
 */
class WaveshaperProcessorRegressionTest {
    ...
    void waveshaper_sineSweep_allPresets() throws Exception {
        for (DspRegression spec : getClass()
                .getDeclaredMethod("waveshaper_sineSweep_allPresets")
                .getAnnotationsByType(DspRegression.class)) {
            WaveshaperProcessor proc = new WaveshaperProcessor(1, TestSignals.SAMPLE_RATE);

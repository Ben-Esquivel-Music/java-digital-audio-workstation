...
/**
 * Golden-file regression test for {@link AcousticReverbProcessor}. Exercises the
 ...
 * goldens with {@code mvn -Pdsp-rebaseline test -Dtest=AcousticReverbProcessorRegressionTest}.</p>
 */
class AcousticReverbProcessorRegressionTest {
    ...
    void acousticReverb_sineSweep_allPresets() throws Exception {
        for (DspRegression spec : getClass()
                .getDeclaredMethod("acousticReverb_sineSweep_allPresets")
                .getAnnotationsByType(DspRegression.class)) {
            AcousticReverbProcessor proc = new AcousticReverbProcessor(1, TestSignals.SAMPLE_RATE);

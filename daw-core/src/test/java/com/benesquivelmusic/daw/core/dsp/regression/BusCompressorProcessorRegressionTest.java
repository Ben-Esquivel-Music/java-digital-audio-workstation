...
/**
 * Golden-file regression test for {@link BusCompressorProcessor}. Exercises the
 ...
 * goldens with {@code mvn -Pdsp-rebaseline test -Dtest=BusCompressorProcessorRegressionTest}.</p>
 */
class BusCompressorProcessorRegressionTest {
    ...
    void busCompressor_sineSweep_allPresets() throws Exception {
        for (DspRegression spec : getClass()
                .getDeclaredMethod("busCompressor_sineSweep_allPresets")
                .getAnnotationsByType(DspRegression.class)) {
            BusCompressorProcessor proc = new BusCompressorProcessor(1, TestSignals.SAMPLE_RATE);

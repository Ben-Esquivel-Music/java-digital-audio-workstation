...
/**
 * Golden-file regression test for {@link MultibandCompressorProcessor}. Exercises the
 ...
 * goldens with {@code mvn -Pdsp-rebaseline test -Dtest=MultibandCompressorProcessorRegressionTest}.</p>
 */
class MultibandCompressorProcessorRegressionTest {
    ...
    void multibandCompressor_sineSweep_allPresets() throws Exception {
        for (DspRegression spec : getClass()
                .getDeclaredMethod("multibandCompressor_sineSweep_allPresets")
                .getAnnotationsByType(DspRegression.class)) {
            MultibandCompressorProcessor proc = new MultibandCompressorProcessor(1, TestSignals.SAMPLE_RATE, new double[]{200.0, 2000.0});

...
/**
 * Golden-file regression test for {@link DeEsserProcessor}. Exercises the
 ...
 * goldens with {@code mvn -Pdsp-rebaseline test -Dtest=DeEsserProcessorRegressionTest}.</p>
 */
class DeEsserProcessorRegressionTest {
    ...
    void deEsser_sineSweep_allPresets() throws Exception {
        for (DspRegression spec : getClass()
                .getDeclaredMethod("deEsser_sineSweep_allPresets")
                .getAnnotationsByType(DspRegression.class)) {
            DeEsserProcessor proc = new DeEsserProcessor(1, TestSignals.SAMPLE_RATE);

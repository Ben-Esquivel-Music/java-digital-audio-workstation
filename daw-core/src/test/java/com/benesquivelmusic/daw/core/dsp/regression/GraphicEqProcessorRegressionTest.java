...
/**
 * Golden-file regression test for {@link GraphicEqProcessor}. Exercises the
 ...
 * goldens with {@code mvn -Pdsp-rebaseline test -Dtest=GraphicEqProcessorRegressionTest}.</p>
 */
class GraphicEqProcessorRegressionTest {
    ...
    void graphicEq_sineSweep_allPresets() throws Exception {
        for (DspRegression spec : getClass()
                .getDeclaredMethod("graphicEq_sineSweep_allPresets")
                .getAnnotationsByType(DspRegression.class)) {
            GraphicEqProcessor proc = new GraphicEqProcessor(1, TestSignals.SAMPLE_RATE);

...
- **Selector reality (vs. the Goals bullet list):** the Goals reference a few selectors by their *intended* names; the shipped selectors are:
  - `.track-strip` (story 270) / `.mixer-channel-strip` (story 271) compute their size in their Java `Skin`, not CSS — they are bridged through `DensityMode.resolveFor` and intentionally have **no** `-fx-padding` density rule.
  - `.browser-list-row` does not exist — the real selector is `.browser-list .list-cell` (story 275).
  - `.inspector-section-row` does not exist — story 272's `InspectorSection` exposes `.inspector-section-header` / `.inspector-section-body`, styled by the `InspectorDrawer`'s own component user-agent stylesheet (`inspector/inspector.css`). The density rules are added to `styles.css` (AUTHOR origin) and override that USER_AGENT sheet by CSS origin; header and body keep separate rules because their baselines differ, and each Comfortable value restates the `inspector.css` baseline verbatim so the default density stays a byte-identical no-op (`DensityInspectorTest`).

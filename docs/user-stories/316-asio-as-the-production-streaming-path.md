...
- **ASIO open failure produces honest state**: the fallback ladder (ASIO → PortAudio → Java Sound) is explicit and loud — at OPEN time the engine publishes one fallback event per hop the open did not take (a request the availability/streaming gate refused before the ladder was built, then every rung that failed before one opened), each naming the requested endpoint and the endpoint that carried the stream. No substitution is silent. This is an event about an open, not a standing comparison: with the transport stopped nothing is *active* at all (book §3.2), which is the ordinary state and publishes nothing.

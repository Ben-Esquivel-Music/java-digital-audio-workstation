...
Implemented; **full `daw-app` suite 2605 green, 0 failures** (3 pre-existing snapshot skips). Uncommitted for review.

**Guards (§1.4).** Four cascade guards deleted via the faithful single-writer restructure: `updating` (`UndoHistoryPanel`) and `updatingControls` (`KeyboardProcessorView`) → stateless echo-guards (the story-291 `now==current` pattern); `programmaticDimensionUpdate` (`TelemetrySetupPanel`) → per-field last-written sentinels; `suppressChangeEvents` (`AudioSettingsDialog`) → explicit reference-counted listener detach/attach around the population methods (the echo-guard can't apply — a backend switch repopulates with genuinely different values). `suppressNotification` (`BinauralMonitorPluginView`) is **kept** as the one justified survivor — it gates an *outbound* host callback / project-dirty side effect during silent restore & refresh, not a self-cascade (`ReentrancyGuardRemovalTest` allows exactly this one, asserting the file documents it).

**Refresh methods (§4.3).** All ten `update*/refresh*/sync*` methods are gone from `MainController`. Where the 290-292 VMs carry the fact, the control is now a real binding: tempo label ← `TransportVM.tempo`; undo/redo buttons ← `HistoryVM` (via `HistoryControlBinder`); project-name cells ← `ProjectVM.name`; arrangement placeholder ← `ProjectVM.getTracks()` emptiness. A new `rebuildViewModels()` builds the VM generation (disposing the prior one) at startup and in `handleProjectRebuild()`/snapshot-restore, so bindings survive the project swap. Where the current VMs carry **no** fact (audio format, lock-badge, checkpoint text, the playhead+ruler-grid which has no discrete snap/grid trigger, and the canvas *time-range* selection which `SelectionVM` does not model), the method was retired behavior-preservingly by inline/rename (`updateProjectInfo`→`applyProjectInfoLabels`, `refreshLockStatusIndicator`→`repaintLockIndicator` poll, `updateCheckpointStatus`→inlined static text, `updatePlayheadFromTransport`/`syncLoopRegionToCanvas`→`tickArrangementOverlays`/`applyLoopAndRulerGrid` per-frame, `refreshArrangementCanvas`→`repaintArrangementCanvas`, `syncSelectionToCanvas`→`paintCanvasSelection`) with a code comment recording why a pure binding awaits a future VM fact (Non-Goal: no new VMs). `MainControllerShrinkTest` asserts MainController declares none of the ten and references none of the 11 removed Hosts.

**Host interfaces (§4.2/§9).** All 11 cascade-feeding Hosts removed; the 14 out-of-scope Hosts (4 framework + the story-294 project/plugin/import set incl. `TrackCreationController.Host`/`AudioImportController.Host`, whose `updateArrangementPlaceholder`/`updateUndoRedoState`/`refreshArrangementCanvas` methods MainController now implements as no-op/redirect) are untouched. Each removed Host's methods became direct constructor deps: **live `Supplier<DawProject>`/`Supplier<UndoManager>` for the init-only controllers** (Tempo/ClipEdit/ClipInteraction/Ripple/History/Menu + the clip-trio) so a project load is reflected without reconstruction, and direct values for the two reconstructed-on-load controllers (Transport/TrackStrip). `ClipEditController`/`ClipInteractionController` + the trim/fade/slip handlers took `Deps` records. `DawMenuBarController.Host` was **split**: the 6 cascade-feeding state queries are deleted (`syncMenuState()` now reads six live `BooleanSupplier`s — the §6.9 derive-don't-poke), and the ~48 action methods (legitimate §2.8 intent routing, not a cross-surface cascade) live on a renamed `MenuActions` interface. `HostCallbackRemovalTest` pins the removal + framework preservation + a typed-bus-event round-trip.

**Tests.** `MainControllerShrinkTest`, `ReentrancyGuardRemovalTest`, `SingleWriterNoLoopTest` (a §4.4 invocation-counter proof that the bound control round-trip re-enters its handler exactly once), `HostCallbackRemovalTest` — all green.

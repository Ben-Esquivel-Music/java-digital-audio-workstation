...
Story 168 (implemented) provides ISRC + CD-Text metadata (`AlbumTrackMetadata`, `CdText`, `IsrcValidator`); story 025 (implemented) provides album assembly (`AlbumSequence`, `AlbumMetadata`, `AlbumTrackEntry`). This story is the serializer.
...
- Consume `AlbumSequence` / `AlbumTrackEntry` (story 025) for track boundaries and `AlbumTrackMetadata` (story 168) for ISRCs + CD-Text.

...
Story 113 (implemented) provides reflective parameter binding for automation via `ReflectiveParameterBinder` and `PluginParameterTarget` in `com.benesquivelmusic.daw.core.automation`; this story reuses those bindings to route a MIDI CC stream into the same sinks.
...
- `MidiMappingManager` maintains `List<MidiMapping>` and consumes `Flow.Publisher<MidiEvent>` from the active MIDI backend; on each CC event, look up matching mappings and write to the target parameter via the story-113 `ReflectiveParameterBinder` / `PluginParameterTarget` API.

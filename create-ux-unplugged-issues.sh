#!/usr/bin/env bash
#
# Creates GitHub issues for the UX & Unplugged Behavior epic.
# Requires: gh CLI authenticated with the repository.
#
# Usage:
#   chmod +x create-ux-unplugged-issues.sh
#   ./create-ux-unplugged-issues.sh
#
# Each issue is labeled with "ux" and "unplugged-behavior". The script
# creates the labels if they do not already exist.
#
# Source: docs/ux-unplugged-behavior-issues.md
#
...
LABEL_UX="ux"
LABEL_UNPLUGGED="unplugged-behavior"

echo "==> Ensuring labels exist..."
gh label create "${LABEL_UX}" \
  --repo "${REPO}" \
  --description "User experience issues" \
  --color "d4c5f9" \
  2>/dev/null || echo "    (label '${LABEL_UX}' already exists)"

gh label create "${LABEL_UNPLUGGED}" \
  --repo "${REPO}" \
  --description "Unplugged/non-functional behavior in the UI" \
  --color "f9d0c4" \
  2>/dev/null || echo "    (label '${LABEL_UNPLUGGED}' already exists)"
...
# Issue 1
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Recording Pipeline Not Connected to Audio Engine" \
  --body "## Description
...
echo "  [1/15] Created: Recording Pipeline Not Connected to Audio Engine"

# ---------------------------------------------------------------------------
# Issue 2
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Add Audio Input Port Selection Dialog for Tracks" \
  --body "## Description
...
echo "  [2/15] Created: Add Audio Input Port Selection Dialog for Tracks"

# ---------------------------------------------------------------------------
# Issue 3
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Track Context Menu Operations Are Stubs" \
  --body "## Description
...
echo "  [3/15] Created: Track Context Menu Operations Are Stubs"

# ---------------------------------------------------------------------------
# Issue 4
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Track Context Menu Items Not Conditionally Disabled" \
  --body "## Description
...
echo "  [4/15] Created: Track Context Menu Items Not Conditionally Disabled"

# ---------------------------------------------------------------------------
# Issue 5
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "User Action Feedback Is Insufficient" \
  --body "## Description
...
echo "  [5/15] Created: User Action Feedback Is Insufficient"

# ---------------------------------------------------------------------------
# Issue 6
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Editor View Tool Buttons Are Non-Functional" \
  --body "## Description
...
echo "  [6/15] Created: Editor View Tool Buttons Are Non-Functional"

# ---------------------------------------------------------------------------
# Issue 7
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Audio Editor Trim and Fade Buttons Are Non-Functional" \
  --body "## Description
...
echo "  [7/15] Created: Audio Editor Trim and Fade Buttons Are Non-Functional"

# ---------------------------------------------------------------------------
# Issue 8
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Loop Mode Not Connected to Transport Engine" \
  --body "## Description
...
echo "  [8/15] Created: Loop Mode Not Connected to Transport Engine"

# ---------------------------------------------------------------------------
# Issue 9
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Mixer Send Level Slider Not Wired to Audio Routing" \
  --body "## Description
...
echo "  [9/15] Created: Mixer Send Level Slider Not Wired to Audio Routing"

# ---------------------------------------------------------------------------
# Issue 10
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Phase Invert Toggle Has No Backing Model State" \
  --body "## Description
...
echo "  [10/15] Created: Phase Invert Toggle Has No Backing Model State"

# ---------------------------------------------------------------------------
# Issue 11
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Snap-to-Grid Flag Not Applied to Editing Operations" \
  --body "## Description
...
echo "  [11/15] Created: Snap-to-Grid Flag Not Applied to Editing Operations"

# ---------------------------------------------------------------------------
# Issue 12
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Sidebar Home, Search, and Help Buttons Lack Actions" \
  --body "## Description
...
echo "  [12/15] Created: Sidebar Home, Search, and Help Buttons Lack Actions"

# ---------------------------------------------------------------------------
# Issue 13
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Settings Dialog Changes Not Applied to Running Engine" \
  --body "## Description
...
echo "  [13/15] Created: Settings Dialog Changes Not Applied to Running Engine"

# ---------------------------------------------------------------------------
# Issue 14
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Track Export Operations Are Stubs" \
  --body "## Description
...
echo "  [14/15] Created: Track Export Operations Are Stubs"

# ---------------------------------------------------------------------------
# Issue 15
# ---------------------------------------------------------------------------
gh issue create --repo "${REPO}" --label "${LABEL_UX}" --label "${LABEL_UNPLUGGED}" \
  --title "Top Toolbar Layout Inconsistent with Design Screenshots" \
  --body "## Description
...
echo "  [15/15] Created: Top Toolbar Layout Inconsistent with Design Screenshots"

echo ""
echo "==> All 15 UX & Unplugged Behavior issues created successfully!"
